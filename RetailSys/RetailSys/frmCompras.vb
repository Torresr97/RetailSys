...
Public Class frmCompras
    ...
    Dim sqlProveedores = "SELECT idproveedor, proveedor FROM proveedores"
    Dim daProveedores As New SqlDataAdapter

    Public Sub New()
        InitializeComponent()
        daProveedores = New SqlDataAdapter(sqlProveedores, My.Settings.cnxDB)
    End Sub

    Private Sub frmCompras_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        daProveedores.Fill(ds, "proveedores")
    ...
    Private Sub btnAplicar_Click(sender As Object, e As EventArgs) Handles btnAplicar.Click
        If MsgBox("Desea aplicar la transaccion", MsgBoxStyle.OkCancel, "Continuar") = MsgBoxResult.Ok Then
            If txtNumeroDocto.Text <> "" Then
                Dim sqlEncabezado = "INSERT INTO compras (idproveedor,idusuario,nodocumento,fecha,descuento,impto,subtotal,total) " &
                             "VALUES (@idproveedor,'admin',@nodocumento,@fecha,0,0,0,0)"

                Dim sqlDetalle = "INSERT INTO compradetalle(idcompra, idarticulo, costo, cantidad, descuento, impto, subtotal, total) " &
                      "VALUES (@idcompra, @idarticulo, @costo, @cantidad, @descuento, @impto, @subtotal, @total)"
                ...
                Dim sqlkardex = "INSERT INTO kardex (idarticulo,entrasale,tipotrasaccion,fecha,cantidad)" &
                               "VALUES (@idarticulo,'1','COMPRA',@fecha,@cantidad)"
                ...
                Using con As New SqlClient.SqlConnection(My.Settings.cnxDB)
                    Try
                        ...
                        cmdEncabezado.Parameters.AddWithValue("@idproveedor", cboProveedor.SelectedValue)
                        cmdEncabezado.Parameters.AddWithValue("@nodocumento", txtNumeroDocto.Text)
                        cmdEncabezado.Parameters.AddWithValue("@fecha", dtpFechaCompra.Value)

                        cmdEncabezado.ExecuteNonQuery()
                        Dim idCompra As Integer = cmdId.ExecuteScalar()
                        ...
                        For Each arti As DataGridViewRow In grdDetalle.Rows
                            ...
                            cmdkardex.Parameters.AddWithValue("@fecha", dtpFechaCompra.Value)
                            cmdkardex.Parameters.AddWithValue("@cantidad", arti.Cells(2).Value)
